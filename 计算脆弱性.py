        ...
        for i in range(height):
            for j in range(width):
                if maxIg[i][j] !=ndv and Igs[page+1][i][j] !=ndv:
                    deltaY[page][i][j] = y0*beta_i[page] * (maxIg[i][j]-Igs[page+1][i][j])
                else:
                    deltaY[page][i][j] = ndv
    time_end = time.perf_counter()
    print("The calculation time is "+str(time_end-time_start)+"s.")
            ...
            with tqdm(filenames) as t:
                for filename in t:
                    if filename[-4:] == ".tif":
                        ...
                        Ig,im_geotrans,im_proj=openSingleImage(path_inputraster)
                        height,width=Ig.shape
                        height_split=[0,height//2,height-1]
                        width_split=[0,width//4,width//2,width//4*3,width-1]
                        index=0
                        for i in range(2):
                            for j in range(4):
                                writeTiff(Ig[height_split[i]:height_split[i+1],width_split[j]:width_split[j+1]],im_geotrans,im_proj,ndv,path_diroutputs[index],filename[:-4]+"_"+str(index)+".tif")
                                index=index+1
...
def main():
    inputpath = "I:\\data_100m\\4_HTEMPX"  # 输入的城市地址
    filename_mode = "MFT"
    filename_max = "max"
    ...
    with tqdm(regionnames) as t:
        ...
        for region in t:
            ...
            regionname_split={}
            #判断是否需要裁剪
            path_clipshp = base_path+"\\shp\\"+regionname+".shp"
            ...
            for ifn,regionname in enumerate(regionname_split):
                ...
                # 打开影像
                path_folder=path_folder+"_"+str(ifn) if bsplit else path_folder
                ...
                path_totalPop = path_diroutput+"_"+countryname+"_2015.tif"
                ...
                path_dirdeltaYpeople = base_path+"\\deltaYpeople1\\"+regionname
